    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for key in newPosDist.keys():
                newBeliefs[key] = newBeliefs[key] + \
                    newPosDist[key]*self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghost in self.particles:
            weight_dist[ghost] = weight_dist[ghost] + \
                self.getObservationProb(observation, cur_pos, ghost, jail_pos)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        legalPositions = list(itertools.product(
            self.legalPositions, repeat=self.numGhosts))
        random.shuffle(legalPositions)
        numLegal = len(legalPositions)
        for i in range(self.numParticles):
            self.particles.append(legalPositions[i % numLegal])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        weight_dist = DiscreteDistribution()
        for ghost in self.particles:
            weight = functools.reduce(lambda i, j: self.getObservationProb(observation[i], cur_pos, ghost[i], self.getJailPosition(
                i)) * self.getObservationProb(observation[j], cur_pos, ghost[j], self.getJailPosition(j)), range(self.numGhosts))
            weight_dist[ghost] += weight
